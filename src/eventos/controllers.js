...
export function viewEventos(req, res) {
  ...
  if (artistaFiltro) {
    ...
    eventos = eventos.filter((evento) =>
      idsEventosDelArtista.includes(evento.id)
    );
  }
...
export function viewEvento(req, res, next) {
  ...
  try {
    ...
    if (err instanceof EventoNoEncontrado) {
      return res
        .status(404)
        .render("pagina", { contenido: "paginas/error", mensaje: err.message });
    }
    next(err);
  }
}
...
export async function agregarEvento(req, res, next) {
  // permisos…
  if (!req.session.esEmpresa && !req.session.esAdmin)
    return res
      .status(403)
      .render("pagina", { contenido: "paginas/error", mensaje: "Sin permiso" });
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    req.log.warn(
...
export function buscarEvento(req, res, next) {
  ...
  try {
    ...
    res.render("pagina", {
      contenido: "paginas/resultadosBusqueda",
      session: req.session,
      eventos,
    });
  } catch (err) {
...
export function viewEventosPasados(req, res) {
  ...
  const eventosConInfo = eventos.map((evento) => {
    const valoracionesEvento = valoraciones.filter(
      (v) => v.id_evento === evento.id
    );
    ...
    const haValorado = idUsuario
      ? valoracionesEvento.some((v) => v.id_usuario === idUsuario)
      : false;
